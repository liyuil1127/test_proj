...
class glean_event(nn.Module):
    ...
    def forward(self, t_list, true_prob_r,time_triples ):
        pred, idx, _ = self.__get_pred_embeds(t_list,time_triples)
    ...
    def predict(self, t_list, true_prob_r, time_triples=0):
    ...
    def predict_new(self, t_list, true_prob_r, time_triples):

        self.layer_norm = True
        evolve_embs = self.word_embeds
        r_emb = self.rel_embeds
        ...
        sorted_t, idx = t_list.sort(0, descending=True)
        if true_prob_r is not None:
            loss = self.criterion(score_rel_new, true_prob_r[idx])
        else:
            loss = None
        return loss, score_rel_new, None

    def evaluate(self, t, true_prob_r, time_triples):
        loss, pred, _ = self.predict_new(t, true_prob_r, time_triples)
        loss_1, pred_1, _ = self.predict(t, true_prob_r, time_triples)
        prob_rel = self.out_func(pred.view(-1))
        prob_rel_1 = self.out_func(pred_1.view(-1))

        sorted_prob_rel, prob_rel_idx = prob_rel.sort(0, descending=True)
        sorted_prob_rel_1, prob_rel_idx_1 = prob_rel_1.sort(0, descending=True)

        sorted_prob_rel = torch.tensor(sorted_prob_rel, dtype=torch.float)
        if torch.cuda.is_available():
            sorted_prob_rel = torch.where(sorted_prob_rel > self.threshold, sorted_prob_rel, torch.zeros(sorted_prob_rel.size()).cuda())
        else:
            www = torch.where(sorted_prob_rel_1 > self.threshold, sorted_prob_rel_1, torch.zeros(sorted_prob_rel_1.size()))
