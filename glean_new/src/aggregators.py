...
class aggregator_event(nn.Module):
    ...
    def forward(self, t_list, ent_embeds, rel_embeds, word_embeds, graph_dict, word_graph_dict, ent_map, rel_map):
        ...
        if self.w_init == 'rand':
            self.w = Parameter(torch.randn(self.city_num, self.group_num).to(device, non_blocking=True),
                               requires_grad=True)
        elif self.w_init == 'group':
            self.w = Parameter(self.new_x, requires_grad=True)
        if self.mode == 'ag':
            self.decoder = DecoderModule(self.x_em, self.edge_h, self.gnn_h, self.gnn_layer, self.city_num,
                                         self.group_num, device)
