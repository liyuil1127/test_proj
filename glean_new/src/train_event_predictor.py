...
#parser.add_argument("--thread", type=int, default=true_params["thread"], help='number of runs')
parser.add_argument("--thread", type=int, default=0.65, help='number of runs')
...
while iterations < args.runs:
    ...
    @torch.no_grad()
    def evaluate(data_loader, dataset_loader, set_name='valid', thread = -1, is_use_th = 0):
        model.eval()
        true_rank_l = []
        prob_rank_l = []
        ...
        for i, batch in enumerate(tqdm(data_loader)):
            ...
            true_rank, prob_rank, loss, ranks = model.evaluate(batch_data, true_r, time_triples, thread, is_use_th)
    ...
    def train(data_loader, dataset_loader, thread= -1):
        model.train()
        total_loss = 0
        t0 = time.time()
        for i, batch in enumerate(tqdm(data_loader)):
    ...
    try:
        ...
        for epoch in range(1, args.max_epochs+1):
            ...
            valid_loss, recall, f1, f2, hits, mrr, mr = evaluate(
                valid_loader, valid_dataset_loader, set_name='Valid', thread=args.thread, is_use_th=1)
            # eval on valid set
    ...
    hloss, recall, f1, f2, hits, mrr, mr = evaluate(test_loader, test_dataset_loader, set_name='Test', thread=args.thread, is_use_th=1)
    print(args)
...
nni.report_final_result(hits_avg[0])
